...
// 		document.baseURI = document.baseURI ? document.baseURI : baseHref ;
	...
	invokeJson : function ( url , jsopCbPg ) 
	{
...
// 			url += "/" ;
// 		} ;
		var timeStamp = ( new Date() ).Format( "yymmdd_ss_SS" ) 
						+ parseInt( Math.random( 9 ) * 10 ) ;
		var optJsonpCbKey = 
		(function ()
		{
			for ( var key in jsopCbPg ) 
		 	{

		 	} ;
		 	var res = ( res = Object.keys( jsopCbPg )[ 0 ] ) ? res : key ;
		 	return res ;
		})() ;
		var jsonpCbKey = optJsonpCbKey ? optJsonpCbKey : "jsonp" ;
		var fnNameExp2 = /(?:\?|&)jsonp=?\w+/ ;
		var fnNameExp = new RegExp( "(?:\\?|&)" + jsonpCbKey + "=?\\w{0,}" , "ig" ) ;
		var fnName = fnNameExp.exec( url )[0] ;

			fnNameExp = new RegExp( "[^?&" + jsonpCbKey + "=]" , "ig" ) ;
			fnName = fnName.match( fnNameExp ) ;
			fnName = fnName ? fnName.join( "" ) : null ;
		
		var jsonpCbVal = ( function()
						 {
						 	for ( var key in jsopCbPg ) 
						 	{

						 	} ;
						 	var res = ( res = jsopCbPg[ optJsonpCbKey ] ) ? res : jsopCbPg[ key ] ;
					 		return res ;
						 })() ;
		var jsonpCbVal = fnName ? fnName : jsonpCbVal ;
		...
		else
		{
			...
			{
				var reg = new RegExp( jsonpCbKey + "=\\w{0,}" , "ig") 
...
// 		url += url.indexOf( "jsonp" ) ? "" : "jsonp" ;
		...
		window.scEleA2 = "script".crtTagEles( { src : repUrl , type : "text/javascript" , } ) ;
